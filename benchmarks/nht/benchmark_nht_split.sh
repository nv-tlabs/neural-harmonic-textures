...
#
# M360 outdoor: 4.5M, 20k iters, per-ray dir encoding
...
COMMON_ARGS="--disable_viewer --render_traj_path $RENDER_TRAJ_PATH"
...
if [ "$METRICS_ONLY" -eq 0 ]; then
    for s in "${M360_OUTDOOR[@]}"; do
        run_scene "$s" "$(get_data_dir $s)" 4 4500000 20000 "" "m360-outdoor"
    done
